...
for om=numel(overlays_max):-1:1
    ...
    cur_plot = plot(D_interp, EFF_interp(:,end+1-counter), sprintf('%s%s%s', lstyle, markerstylevec(mstyleidx), colorvec(coloridx))); % plot one line
    ...
    set(cur_plot, plot_curves_params{:}); % additional plot style
    plot_title = strcat(plot_title, ' - theo error rate');
