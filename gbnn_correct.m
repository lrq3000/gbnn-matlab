...
% IDEE ML pour tags net: on regarde tous les tags uniques et on garde le premier qui forme une clique de c fanaux au moins (c fanaux exactement si overlays_max == 0, sinon on peut avoir plus de fanaux que c par overlap de deux cliques qui ont les mêmes tags).
                    ...
                    if any(finalmsg ~= init(:,mi)) % if it's an error
                        ...
                        dstats.tags_net_max = full(max(nonzeros(net)));
                        dstats.tags_net_min = full(min(nonzeros(net)));
                        ...
                        outm1 = full(out(:,mi)); % propag + filtering fanals, before tags disambiguation
                        ...
                        %dstats.tags_major_gwta_filtered_wrong = full(any(any(decoded_edges == min(dstats.tags_major), 1) - any(decoded_edges(find(gmask), :) == min(dstats.tags_major), 1))); % this version works if using gwta on rows instead of columns
                        ...
                        try
                            ...
                            try
                                tags_compare_side = full([[gmask' out_edges] ; [zeros(size(tags_compare_small',1),1) tags_compare_small']]);
                            catch err
                            end
                        catch err
                        end
                        ...
                        dtotalstats.tags_error = dtotalstats.tags_error + 1;
                    end
