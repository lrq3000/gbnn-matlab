...
M = 0.5:2:20.5; % this is a vector because we will try several values of m (number of messages, which influences the density)
Mcoeff = 1E3;
...
overlays_max = [1 5 0]; % overlays_max = [1 5 20 100 1000 0];
...
% == Launching the runs
D = zeros(numel(M), numel(overlays_max)*numel(C));
E = zeros(numel(M), numel(overlays_max)*numel(C));
...
Coffset = (1:size(E, 2)/numel(C):size(E,2)) - 1;
...
for t=1:statstries
    for c=1:numel(C)
        ...
        for m=1:numel(M) % and for each value of m, we will do a run
            % Launch the run
            if m == 1
                [cnetwork, thriftymessages, density] = gbnn_learn('m', round(M(1, 1)*Mcoeff), 'miterator', miterator(1,m), 'l', l, 'c', C(c), 'Chi', Chi(c), ...
                ...
                [cnetwork, s2, density] = gbnn_learn('cnetwork', cnetwork, ...
                                                            'm', round((M(1, m)-M(1,m-1))*Mcoeff), 'miterator', miterator(1,m), 'l', l, 'c', C(c), 'Chi', Chi(c), ...
                                                            'enable_overlays', enable_overlays, ...
                                                            'silent', silent);
...
for c=numel(C):-1:1
    counter = 1 + Coffset(c); % useful to keep track inside the matrix E. This is guaranteed to be OK since we use the same order of for loops (so be careful, if you move the forloops here in plotting you must also move them the same way in the tests above!)
